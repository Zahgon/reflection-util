...
final class RecordSupport {
    ...
    private static Object[] buildUniqueValues(Class<?> recordClass) {
        return getRecordComponents(recordClass).map(RecordComponent::getType).map(uniqueValueBuilder()).toArray(Object[]::new);
    }
    ...
    private static <T> T safeNumberCast(long currentIndex, T castedValue, long castedValueAsLong, Class<?> valueType) {
        ...
        Assert.isTrue(castedValueAsLong == currentIndex, () -> "Having more than " + (currentIndex - 1) + " record components of type " + valueType.getName() + " is currently not supported");
        return castedValue;
    }
    ...
    private static Method getRecordComponentAccessor(Class<?> recordClass, int componentIndex) {
        return getRecordComponents(recordClass).skip(componentIndex).findFirst().map(RecordComponent::getAccessor).orElseThrow(IllegalStateException::new);
    }
